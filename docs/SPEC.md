...
### 5.1 Quy tắc dữ liệu
- `slug`: chỉ chữ thường không dấu, số, gạch ngang; unique; cấm từ khoá `admin/api/login/preview`.
- `numberOfPeople`: 0–20; nếu `NOT_ATTENDING` ép về 0.
- 1 `submissionKey` → 1 RSVP / 1 wedding (idempotent).
...
- `GET  /api/exports/weddings/[id]/rsvps` (CSV UTF-8)
...
{ "invitationCode":"optional", "fullName":"Nguyễn Văn A", "phone":"09...",
  ...
  "submissionKey":"uuid-tạo-ở-browser" }
```
Luồng: Zod parse → tìm wedding PUBLISHED → check `showRsvp` → hash IP (SHA-256 + secret,
không lưu IP thô) → nếu có `invitationCode` giới hạn theo `maximumPeople` →
upsert theo `(weddingId, submissionKey)` → trả success (không trả RSVP khác).

### 7.2 Lời chúc — `POST /api/public/weddings/[slug]/wishes`
`guestName` 2–120 ký tự, `content` 2–1000 ký tự → check published & `showWishes` →
loại ký tự điều khiển, lưu text thuần → tạo Wish `PENDING`. Public chỉ query `APPROVED`.
...
- Giới hạn đăng nhập sai theo IP (bộ nhớ local → Redis/DB khi online).
